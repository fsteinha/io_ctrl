...
void testInitialState() {
    ...
    ASSERT_EQUAL(EN_DEB_STATE_STOP, debounce.get_state());
}

// Test trigger in count mode
void testTriggerCountMode_3() {
    Debounce debounce(EN_DEB_MODE_COUNT, 100, 3, nullptr);
    ASSERT_EQUAL(EN_DEB_STATE_RUN, debounce.trigger());
    ASSERT_EQUAL(EN_DEB_STATE_RUN, debounce.get_state());
    ASSERT_EQUAL(EN_DEB_STATE_RUN, debounce.trigger());
    ASSERT_EQUAL(EN_DEB_STATE_RUN, debounce.trigger());
...
// Test trigger in count mode with reset
void testTriggerCountMode_reset() {
    Debounce debounce(EN_DEB_MODE_COUNT, 100, 3, nullptr);
    ASSERT_EQUAL(EN_DEB_STATE_RUN, debounce.trigger());
    ASSERT_EQUAL(EN_DEB_STATE_RUN, debounce.get_state());
    ASSERT_EQUAL(EN_DEB_STATE_RUN, debounce.trigger());
...
void runAllTests() {
    ...
    s.push_back(CUTE(testTriggerCountMode_reset));
